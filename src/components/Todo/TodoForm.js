...
class TodoForm extends Component {
	...
	tnameChange(e) {
		let v = e.target.value, reg = /^[a-z\d\-_\s]+$/i, err = '';
		...
		if(err) {
			...
			setTimeout(() => { this.state.errorMsg && this.setState({errorMsg: ''}) }, 7000);
			return;
		}
	...
	render() {
		...
		return (
			...
			<button type="submit" className="btn btn-primary leftBtn" disabled={!this.state.tname || this.state.error}>Add</button>
			{errblock}
			</form>
		);
	}
